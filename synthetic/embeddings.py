...
# takes in a masked sentence as a parameter and embeds each word
def embed(ms):
    sentence = Sentence(ms)

# embed a sentence using glove
    glove_embedding.embed(sentence)

# prints out embedded tokens
...
def mask_sentence(sentence: str):
    ...
    if org_count != 2:
        print("You either have too few or too many organizations in the given sentence.")
        #sys.exit(1)
    ...
    init_sentence = init_sentence.replace(org_list[0], "NEFROM") # change first org to __NE_FROM__
    init_sentence = init_sentence.replace("NEFROM's", "NEFROM") # removing apostrophe s
    masked_sent = init_sentence.replace(org_list[1], "NETOOO") # change second org to __NE_TO__
        ...
        if os.path.isfile(f): # check if path is a file
            with open(f) as txt_file: # open file
                ...
                for line in lines: # loop through the lines in the file
                    ...
                    embed(ms) # print the masked sentence
