...
$(document).ready(function(){
   // $("body").load(function(){})
   	$("#id_select_plan").change(function(){ 	
   		...
   		var selected_plan = $("#id_select_plan").val();
   		//window.alert("piano selezionato " + selected_plan);
   		
   		//controllo che l'utente non abbia selezionato la voce Select a plan
   		if (selected_plan == "") {location.reload();} else {
    ...
    $("#id_button_mapping_last").click(function(){
      ...
      if ($("#id_select_plan").val() != ""){	
        ...
        $.getJSON("get_unit.php?unit="+$("#candidate_mapping_last").val(), function(data, status){
        	//campo_mapping_unit
			//description_mapping_unit
        	$("#campo_mapping_unit").html(data.name);
            $("#description_mapping_unit").html(data.description);
            var plan = $("#id_select_plan").val();
        	////map_plan_unit.php?plan=3$unit=5
        	//window.alert("map_plan_unit.php?plan="+plan+"&unit="+$("#id_candidate_mapping").val());
...
$(document).ready(function(){
    $("#id_goal_delete").click(function(){
    	...
    	if (goal_to_del == "Select a goal"){window.alert("devi selezionare un goal da cancellare")} else {
        ...
        $.getJSON("get_plan.php?plan="+$("#id_select_plan").val(), function(data, status){
        	...
        	if (plan_goal == goal_to_del) {
            	...
            	$.getJSON("detach_goal_from_plan.php?plan="+$("#id_select_plan").val(), function(data, status){
        			//$("#plans").empty();
        			});
            	}
        });
    ...
    $("#id_select_plan").change(function(){ 	
       ...
       $.getJSON("get_plan.php?plan="+plan, function(data, status){       
            ...
            */
            ...
            if (unit != null) {
            	...
            	$.getJSON("get_unit.php?unit="+unit, function(data, status){
            		...
            		//$("#id_mapping_last").val(mapping_i);
    					...
    					$("#campo_mapping_unit").html("___");
    					$("#description_mapping_unit").html("___");
    					$("#id_mapping_last").val();
    					}
            ...
            //window.alert ("piano corrente:" +  $("#id_select_plan").val());
            //get_subplans(plan);
...
$(document).ready(function(){
    $("#id_button_create_deplan").click(function(){
     ...
     if ($("#id_create_deplan").val() != "insert new directly executable plan name") {	
		...
		$.get("create_plan.php?name="+$("#id_create_deplan").val()+"&print="+$("#print_create_deplan").val()+"&type=Base"+"&accomplished="+$("#id_accomplished").val(), function(data, success){
				...
				$("#campo_mapping_unit").html("____");
				$("#description_mapping_unit").html("____");
        	    ...
        	    get_actions(nuovo_id);
        	    document.getElementById('form_incidents').reset(); 
        	    document.getElementById('set_incident').innerHTML = '____';
            	//carico precondizioni e effetti
            	get_states(nuovo_id);        						    
        		});        		      		        		   		
 		});	
...
$(document).ready(function(){
    $("#id_button_delete_plan").click(function(){
    	//window.alert("delete_plan_from_repository.php?plan="+$("#id_select_plan").val());
		$.get("delete_plan_from_repository.php?plan="+$("#id_select_plan").val(), function(data, success){
			location.reload();       
			/*			
...
function get_subplans (plan) {
        ...
        xmlhttp.open("GET","get_actions.php?plan="+plan,false);
        xmlhttp.send();		
}
...
function deleteAction (plan,action){
	...
	//window.alert("tolgo azione " + action + "da piano " + plan );
	//if (document.getElementById('check').value != "no"){
		//document.getElementById('test').innerHTML = "hai selezionato: " + candidate_unit;		
		if (window.XMLHttpRequest) {
            // code for IE7+, Firefox, Chrome, Opera, Safari
...
function getInfoAction (action){
		...
		xmlhttp.onreadystatechange = function() {
            if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                ...
                document.getElementById("id_name_incident").value = my_response.name; 
...
{
    var i;
    //window.alert("rimuovo le voci di " + selectbox.id)
...
function deleteState (state, plan, flag){
		...
		if (state == "select a state") {window.alert("please select a state to be removed from the plan");} else {
...
function deleteStateFromRep (state, plan) {
		...
		//window.alert ("il piano è : " + plan);
		
		if ((state == "select a state") ||  (plan == "") ) {window.alert("please select a state to be deleted");} else {
		...
		xmlhttp.onreadystatechange = function() {
            if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
            	...
            	//window.alert("stato effetti 1: " + document.getElementById("effetti_candidati").value + " - stato effetti 2: " + document.getElementById("effetti_unit").value);
            	
            	//verifico se la precondizione non è già selezionata nell'altro viewer
            	if (document.getElementById("effetti_candidati").value == state || document.getElementById("effetti_unit").value == state) {
				...
				if (document.getElementById("precondizioni_candidate").value == state || document.getElementById("precondizioni_unit").value == state) {
            	...
            	document.getElementById('id_p_viewer').reset(); 
            	document.getElementById('state_name_pre').innerHTML = '_____'; 	
            	document.getElementById('set_state_p_pre').innerHTML = '_____' ;
            	
            	
            	document.getElementById('id_e_viewer').reset(); 
            	document.getElementById('state_name_eff').innerHTML = '_____'; 	
            	document.getElementById('set_state_p_eff').innerHTML = '_____' ;
...
function deleteActionFromRep (action, plan) {
	 ...
	 if (action == "") {window.alert ("please select the action to be deleted");} else {
	
		//window.alert("cancello l'azione " + action);
		//document.getElementById('test').innerHTML = "hai selezionato: " + candidate_unit;		
		if (window.XMLHttpRequest) {
            // code for IE7+, Firefox, Chrome, Opera, Safari
            xmlhttp = new XMLHttpRequest();
		...
		xmlhttp.onreadystatechange = function() {
            if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                ...
                //document.getElementById("cand_incident").value = ''; 
                document.getElementById("id_name_incident").value = '';  
