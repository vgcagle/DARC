...
<title>Drammar: Directly Executable Plan</title>

<link href="plan_template.css" rel="stylesheet" type="text/css" media="screen" />

<script src="https://ajax.googleapis.com/ajax/libs/jquery/1.12.2/jquery.min.js" ></script>

<script src="script_deplans.js" ></script>


</head>

<!--<body echo "<body onload=\"get_units_select (); \" />";>-->
<body>
...
<!-- #########  -->
<script>
var id_unit_corrente = document.getElementById('id_plan_corrente').value; 
...
<!--<div id="titolo"><center>Drammar annotation: direct executable plan</center></div>-->

<div id="titolo"><center><a href="unit_template.php">Units</a> | <a href="absplan_template.php">Abstract Plans</a> | <a href="index.html">Drammar annotation:</a> DIRECTLY EXECUTABLE PLANS </center></div>
...
<!-- ############ ############ ############ --> 
  <div id="plan_header">
    ...
    <div id="plan_header_title"><center>Directly Executable Plan Header</center></div>
    ...
    <form id="plan_header_form" id="plan_header_form"> 
      ...
      <!-- ############ PLAN ############ --> 
      <div id="select_or_create_deplan">
      ...
      <!-- ############ MAPPING ############ --> 
      <div id="mapping">
        <center>mapping unit</center><br/>
        <input type="hidden" id="id_mapping_last"  />
        <!--<select id="candidate_mapping_last"><option>Select a unit</option></select><input type="button" id="id_button_mapping_last" value="+" />-->
        <table style="text-align:left"><col width="20%"><col width="80%">
          <tr class="Testo"><td>name</td><td><span id="campo_mapping_unit" />___</span></td></tr>
          <tr class="Testo"><td>description</td><td><span id="description_mapping_unit">___</span></td></tr>
        </table> 
      </div> <!-- div id="mapping" -->
...
<!-- ############ ############ ############ --> 
  <div id="plan_content">
    <div id="plan_content_title"><center>Directly Executable Plan Content</center></div>
    
    <form id="plan_content_form"> 
<!-- ############ ############ ############ --> 
<!--  ############ PRECONDITIONS ########## -->
<!-- ############ ############ ############ --> 
    <div id="preconditions">
      <form name="deplan_preconditions_form" id="deplan_preconditions_form">
        ...
        onclick="var plan = document.getElementById('id_select_plan').value; var state = document.getElementById('precondizioni_candidate').value; addState(state, plan,'pre');" />         
		...
		<input type="button" value="-"
        onclick="var plan = document.getElementById('id_select_plan').value; var state = document.getElementById('precondizioni_unit').value; deleteState(state, plan,'pre');" />
        <br/><br/>
<!--  ############ PRECONDITION STATE ########## -->
        <!--Precondition state -->
      ...
      <form id="id_p_viewer" name="p_viewer">
        ...
        <center>
          <input type="button" value="DELETE FROM REPOSITORY" onclick="var state =  document.getElementById('precondizioni_candidate').value; var plan = document.getElementById('id_select_plan').value;  deleteStateFromRep(state, plan);" />
          ...
          <input type="button" value="CREATE NEW" onclick="var plan = document.getElementById('id_select_plan').value; var my_print = document.getElementById('print_state_p_pre').value; createState(document.p_viewer.id_state_pre.value,document.p_viewer.id_type_state_p_pre.value, document.p_viewer.id_status_state_p_pre.value, my_print, plan, 'pre');" />
        </center>
        </div>
      </form>
    </div> <!-- preconditions -->
...
<!-- ############ ############ ############ --> 
      <div id="incidents">
        ...
        <form id="form_incidents" name="incidents">
          <select  id="cand_incident" name="add_incident" <?php  echo "onChange=\"getInfoAction(this.value);\""; ?> >
		    <option>Select an action</option>
		</select>
		<input type="button" value="+" id="id_button_addaction" <?php  // echo "onclick=\"var id_unit_corrente = document.getElementById('id_unit_corrente').value; var id_timeline_corrente = document.getElementById('id_timeline_corrente').value; addAction(document.incidents.add_incident.value,id_unit_corrente,document.incidents.add_incident.text);\""; ?>  
		/> 
		
		<!--azione <span id="id_current_action">___</span>-->

        <!--
        <select id="curr_incident" name="list_incidents" <?php echo "onChange=\"getInfoAction(this.value);\""; ?>>
          <option>current actions</option></select>
        
          
         
        <input type="button" value="-" onclick="deleteAction(document.getElementById('id_select_plan').value, document.getElementById('curr_incident'));" />
        -->  
        <br/><br/>
        <!--Action: <span id="cur_incident">___</span>--> 
        <table style="text-align:left"><col width="30%"><col width="70%">
          <!-- tr class="Testo"><td>name</td><td><input  name="id_incident" id="id_id_name" type="text" /></td></tr -->
          <tr class="Testo"><td>name</td><td><input  name="name_incident" id="id_name_incident" type="text" size="75" /></td></tr>
          <tr class="Testo"><td>description</td><td><input  name="print_incident" id="id_print_incident" type="text" size="75" /></td></tr>
          <tr class="Testo"><td>set</td><td><span id="set_incident">___</span></td></tr>
        </table> 
        <center>        
          <input type="button" value="DELETE FROM REPOSITORY" onclick="deleteActionFromRep(document.getElementById('id_hidden_action').value, document.getElementById('id_select_plan').value)" />
          <input type="hidden" id="id_hidden_action" />
          <input type="button" value="CLEAR" onclick="document.getElementById('form_incidents').reset(); document.getElementById('set_incident').innerHTML = '____'" />
          <input type="button" value="CREATE & ADD NEW ACTION" onclick="createAction(document.getElementById('id_name_incident').value, document.getElementById('id_print_incident').value, document.getElementById('id_select_plan').value);" />
...
<!-- ############ ############ ############ --> 
    <div id="effects">
      ...
      onclick="var plan = document.getElementById('id_select_plan').value; var state = document.getElementById('effetti_candidati').value; addState(state,plan,'eff');"  />        

        <br/>
        Effect states
		<select name="select_effect" id="effetti_unit" onChange="getInfoState(this.value,'eff');"><option>current effects</option></select>
       <input type="button" value="-" onclick="var plan = document.getElementById('id_select_plan').value; var state = document.getElementById('effetti_unit').value; deleteState(state,plan,'eff');" />
        <br/><br/>
<!--  ############ EFFECT STATE ########## -->
        <!--Effect state -->
      ...
      <form id="id_e_viewer" name="e_viewer">
        ...
        <center>
          <input type="button" value="DELETE FROM REPOSITORY" onclick="var state = document.getElementById('effetti_candidati').value; var plan = document.getElementById('id_select_plan').value; deleteStateFromRep(state, plan); " />
          ...
          <input type="button" value="CREATE NEW"  onclick="var plan = document.getElementById('id_select_plan').value; var my_print = document.getElementById('print_state_p_eff').value; var state= document.getElementById('id_state_p_no_write_eff').value; createState(state, document.e_viewer.id_type_state_p_eff.value, document.e_viewer.id_status_state_p_eff.value, my_print, plan, 'eff');"/>
        </center>
        </div>
        </form>
      </div> <!-- effects -->
